        ...
        for proc in psutil.process_iter(['pid', 'name', 'cmdline', 'create_time']):
            try:
                if (proc.info['cmdline'] and 
                    ...
                    # 创建进程唯一标识符 (PID + 创建时间)
                    process_key = f"{proc.info['pid']}_{proc.info.get('create_time', 0)}"
                    if process_key in seen_processes:
                        continue  # 避免重复处理同一个进程
                    ...
                    if existing_task:
                        # 如果任务已存在，更新状态而不是创建新任务
                        existing_task['status'] = 'running'
                        existing_task['pid'] = proc.info['pid']
                        running_tasks.append(existing_task)
                    else:
                        # 提取参数
    ...
    def start_task(self, interval_seconds=30, records_per_batch=10, selected_tables=None):
        ...
        try:
            ...
            for task in running_tasks:
                if (task['interval_seconds'] == interval_seconds and 
                    task['records_per_batch'] == records_per_batch and 
                    set(task['tables']) == set(selected_tables or [])):
                    return False, f"相同配置的任务已在运行 (任务ID: {task['task_id']})", None
...
def main():
    ...
    with st.sidebar:
        ...
        # 添加防重复点击机制
        if 'last_task_start_time' not in st.session_state:
            st.session_state.last_task_start_time = 0
        
        if st.button("▶️ 启动新后台任务"):
            current_time = time.time()
            # 防止重复点击：3秒内只能点击一次
            if current_time - st.session_state.last_task_start_time < 3:
                st.warning("请勿频繁点击，请等待3秒后再试")
            else:
                st.session_state.last_task_start_time = current_time
