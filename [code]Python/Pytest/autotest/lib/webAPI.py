...
class APIMgr:
    ...
    def mgr_login(self,username='byhy',password='88888888',useProxy=False):
        ...
        response = self.s.post("http://127.0.0.1:8000/mgr/sign.html/",
                                 data={
                                     'username': username,
                                     'password': password
                                 }
    ...
    def customer_list(self,pagesize=10,pagenumber=1,keywords=''):
        ...
        print('列出客户')
        response = self.s.get("http://127.0.0.1:8000/api/mgr/customers",
              params={
    ...
    def customer_add(self,name,phonenumber,address):
        print('添加客户')
        response = self.s.post("http://127.0.0.1:8000/api/mgr/customers",
              json={
                    "action":"add_customer",
                    "data":{
                        "name":name,
    ...
    def customer_del(self,cid):
        print('删除客户')
        # 'http://127.0.0.1:8000/mgr/sign.html'
        response = self.s.delete("http://127.0.0.1:8000/api/mgr/customers",
              json={
                    "action":"del_customer",
                    "id": cid
                })
    ...
    def medicine_list(self,pagesize=10,pagenumber=1,keywords=''):
        print('列出药品')
        response = self.s.get("http://127.0.0.1:8000/api/mgr/medicines",
              params={
    ...
    def medicine_del(self,mid):
        print('删除药品')
        response = self.s.delete("http://127.0.0.1:8000/api/mgr/medicines",
              json={
                    "action":"del_medicine",
                    "id": mid
                })
    ...
    def order_list(self,pagesize=10,pagenumber=1,keywords=''):
        print('列出订单')
        response = self.s.get("http://127.0.0.1:8000/api/mgr/orders",
              params={
                  'action' :'list_order',
                  'pagesize' :pagesize,
                  'pagenum' :pagenumber,
    ...
    def order_del(self,oid):
        print('删除订单')
        response = self.s.delete("http://127.0.0.1:8000/api/mgr/orders",
              json={
                    "action":"delete_order",
                    "id": oid
                })
